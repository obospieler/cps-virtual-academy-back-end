...
class FileMakerService {
    ...
    private handleError(error: any, operation: string): never {
        const errorMessage = error.response?.data?.messages?.[0]?.message || error.message;
     ...
     */
    async makeAuthenticatedRequest<T>(requestFn: (token: string) => Promise<T>): Promise<T> {
     ...
     */
    async create(layout: string, data: any = {}, parameters: FileMakerParameters = {}): Promise<FileMakerResponse> {
        if (!layout) {
        ...
        return await this.makeAuthenticatedRequest(async (token) => {
            console.log(`${this.formatLogTime(new Date())} make request to filemaker `, {
     ...
     */
    async edit(layout: string, recordId: string, data: any, parameters: FileMakerParameters = {}): Promise<FileMakerResponse> {
        if (!layout) {
            throw new Error('Layout name is required');
        }
        if (!recordId) {
        ...
        return await this.makeAuthenticatedRequest(async (token) => {
            ...
            if (parameters.merge) {
                return {
                    success: true,
                    ...Object.assign(data, { recordId: recordId }, response.data.response)
                };
            }
     ...
     */
    async delete(layout: string, recordId: string, parameters: FileMakerParameters = {}): Promise<FileMakerResponse> {
        if (!layout) {
            throw new Error('Layout name is required');
        }
        if (!recordId) {
     ...
     */
    async get(layout: string, recordId: string, parameters: FileMakerParameters = {}): Promise<FileMakerResponse> {
        if (!layout) {
            throw new Error('Layout name is required');
        }
        if (!recordId) {
     ...
     */
    async list(layout: string, parameters: FileMakerParameters = {}): Promise<FileMakerResponse> {
        if (!layout) {
            throw new Error('Layout name is required');
        }

        // Sanitize parameters for the request
        const requestParams = toStrings(sanitizeParameters(namespace(parameters), [
            '_limit',
            '_offset',
            '_sort',
            'portal',
     ...
     */
    async find(layout: string, query: any, parameters: FileMakerParameters = {}): Promise<FileMakerResponse> {
        if (!layout) {
            throw new Error('Layout name is required');
        }
        if (!query) {
     ...
     */
    async globals(data: any, parameters: FileMakerParameters = {}): Promise<FileMakerResponse> {
        if (!data || typeof data !== 'object') {
            throw new Error('Valid data object is required');
        }
     ...
     */
    async upload(file: string | FileMakerFile, layout: string, containerFieldName: string, recordId = 0, parameters: FileMakerParameters = {}): Promise<FileMakerResponse> {
        if (!layout) {
            throw new Error('Layout name is required');
        }
        ...
        if (typeof file === 'string') {
            ...
            // If file is an object with buffer and name
            stream = intoStream(file.buffer);
            (stream as any).name = file.name; // Type assertion for the name property
        } else {
            throw new Error('File must be either a path string or an object with buffer and name properties');
        }

        form.append('upload', stream);

        // If recordId is 0, create a new record first
        const resolvedRecordId = recordId === 0 
            ? await this.create(layout, {}).then(response => response.recordId)
            : recordId;

        return await this.makeAuthenticatedRequest(async (token) => {
            console.log(`${this.formatLogTime(new Date())} make request to filemaker `, {
                url: this.buildUrl(`layouts/${layout}/records/${resolvedRecordId}/containers/${containerFieldName}/${parameters.fieldRepetition || 1}`),
                parameters,
                headers: {
                    'Authorization': `Bearer ${token}`,
                    ...form.getHeaders()
            ...
            const response = await this.axiosInstance.post(
                this.buildUrl(`layouts/${layout}/records/${resolvedRecordId}/containers/${containerFieldName}/${parameters.fieldRepetition || 1}`),
                form,
                {
                    headers: {
                        'Authorization': `Bearer ${token}`,
     ...
     */
    async run(layout: string, scripts: FileMakerScript | FileMakerScript[], parameters: FileMakerParameters, request: FileMakerParameters): Promise<FileMakerResponse> {
        if (!layout) {
            throw new Error('Layout name is required');
        }
     ...
     * @returns {Promise<Object>} The script execution result
     */
    async script(layout: string, script: string, param: FileMakerScript['param'] = {}, parameters: FileMakerParameters = {}): Promise<FileMakerResponse> {
        if (!layout) {
            throw new Error('Layout name is required');
        }
        if (!script) {
     ...
     * @returns {Promise<Object>} The script execution result
     */
    async runScript(layout: string, script: string, recordId: string, param: FileMakerScript['param'] = {}, parameters: FileMakerParameters = {}): Promise<FileMakerResponse> {
        if (!layout) {
            throw new Error('Layout name is required');
        }
        if (!script) {
     ...
     */
    uploadUrl(layout: string, recordId: string, fieldName: string, fieldRepetition = 1, version = 'vLatest'): string {
        return this.buildUrl(`layouts/${layout}/records/${recordId}/containers/${fieldName}/${fieldRepetition}`);
    }
     ...
     */
    scriptUrl(layout: string, script: string, parameter: FileMakerScript['param'], version = 'vLatest'): string {
     ...
     */
    runScriptUrl(layout: string, script: string, recordId: string, param: FileMakerScript['param'], version = 'vLatest'): string {
        return this.buildUrl(`layouts/${layout}/records/${recordId}?script=${script}&script.param=${param}`);
    }
     ...
     */
    async databases(credentials: any, version: string): Promise<FileMakerResponse> {
        return await this.makeAuthenticatedRequest(async (token) => {
            console.log(`${this.formatLogTime(new Date())} make request to filemaker `, {
     ...
     */
    async layouts(parameters: FileMakerParameters = {}): Promise<FileMakerResponse> {
        return await this.makeAuthenticatedRequest(async (token) => {
            console.log(`${this.formatLogTime(new Date())} make request to filemaker `, {
     ...
     */
    async scripts(parameters: FileMakerParameters = {}): Promise<FileMakerResponse> {
        return await this.makeAuthenticatedRequest(async (token) => {
            console.log(`${this.formatLogTime(new Date())} make request to filemaker `, {
